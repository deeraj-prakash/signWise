...
import  ProfileCallsToolbar  from 'src/components/_dashboard/user/list/ProfileCallsToolbar';
...
export default function Calls(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = Calls.map((n) => n.date);
  ...
  return (
    <Page title="Calls | SignWise Solutions">
      <Container>
        ...
        <Card>
          <ProfileCallsToolbar numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, date, direction,from, to } = row;
